...
start = time.clock()  # changed to the line below: see https://stackoverflow.com/a/58569410
...
from gridworld import GridworldEnvironment, NStateMdpGaussianFeatures, GridworldMdpWithDistanceFeatures, GridworldMdp
...
if __name__ == '__main__':
    ...
    reward_space_true = np.array(np.random.randint(-9, 10, size=[size_reward_space_true, args.feature_dim]),
                                 dtype=np.int16)
    ...
    # Set up env and agent for gridworld
        ...
        for i in range(args.num_test_envs):
            ...
            mdp = GridworldMdpWithDistanceFeatures(test_grid, test_goals, args, dist_scale, living_reward=-0.01,
                                                   noise=0)
            env = GridworldEnvironment(mdp)
            inference = Inference(
                mdp, env, beta, reward_space_true, reward_space_proxy=[])
